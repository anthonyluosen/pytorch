...
def FC(length,rf, timeseries, lenn=96, na='FC'):
    ...
    Long = []
    Short = []
    for i in tqdm.tqdm(range(len(timeseries) - length)):
        # print(i)
    ...
    length = length
    a = {}
    a['long-short'],a['long'],a['short'] =  Mean[0] / 12, Mean[1] / 12, Mean[2] / 12
...
def output2(length,CLF,name,rf,timeseries2):
    ...
    Long = []
    Short = []
    for i in tqdm(range(len(timeseries2) - length)):
        FINALm = pd.concat(timeseries2[i:(i + length + 1)], axis=0)
        #将因子项里面月收益不为null的因子存在null填充为0
