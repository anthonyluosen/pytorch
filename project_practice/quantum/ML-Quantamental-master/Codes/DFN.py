...
class DFN(nn.Block):
    def __init__(self, outputdim, batch_size=10, epoch=10, depth=1, neuralset=[],
                 activiatemethod='relu', trainmethod='sgd', lr=0.5, **kwargs):
        '''
        主函数，用于深度前馈网络训练
    ...
    def retrain(self,Xnd,Ynd,data_iter,k):
        knew = 0
        self.net.initialize(ctx=se, force_reinit=True)
    ...
    def fit(self,X,Y):
        ...
        for i in range(self.epoch):
            ...
            # self.Trainer.step(4400)
            for xtrain, ytrain in data_iter:
                ytrain = nd.array(ytrain)
                with mx.autograd.record():
                    l = self.loss(self.net(xtrain), ytrain)
                l.backward()
                self.Trainer.step(1)
                print('#', )
            k += 1
            lossout = self.loss(self.net(Xnd), Ynd)
            print('epoch ' + str(i) + ': loss is %f' % lossout.asnumpy())
            if lossout.mean().asnumpy()>1:
                retrainNN = True
                break
            elif np.isnan(lossout.mean().asnumpy()):
