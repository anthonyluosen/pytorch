...
size=pd.read_csv('..\Data\\factor\\01size.csv')
EP=pd.read_csv('..\Data\\factor\\32EP.csv')
...
factoreq=pd.read_csv('..\Data\\factorEW.csv')
factorvw=pd.read_csv('..\Data\\factorVW.csv')
...
for i in range(len(factoreq.columns)-1):
    ...
    ff30=pd.read_csv('..\Data\\ff30.csv')
...
ret = pd.read_csv('..\Data\\final_return.csv')
size = pd.read_csv('..\Data\\factor\\01size.csv')
BM = pd.read_csv('..\Data\\factor\\28BM.csv')
size = size.iloc[:, :-1]
size.columns = ret.columns
path = r'..\Data\\factor'
file = glob.glob(os.path.join(path, "*.csv"))
