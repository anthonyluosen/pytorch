...
def train_one_epoch(loader, model, optimizer, loss_fn, scaler, device):
    losses = []
    # loop = tqdm(loader)
    for batch_idx, (data, targets, _) in enumerate(tqdm(loader)):
...
def main():
    ...
    # )
    # test_ds = DRDataset(
    #     images_folder="D:\database\eye\\test",
    #     path_to_csv="D:\database\eye\\trainLabels.csv",
    #     transform=config.val_transforms,
    #     train=False,
    # )
    # test_loader = DataLoader(
    #     test_ds, batch_size=config.BATCH_SIZE, num_workers=0, shuffle=False
    # )
    ...
    loader = DataLoader(
        dataset=dataset, batch_size=32, num_workers=0, shuffle=True
    )
    ...
    for epoch in range(config.NUM_EPOCHS):
        train_one_epoch(loader, model, optimizer, loss_fn, scaler, config.DEVICE)

        # get on validation
        preds, labels = check_accuracy(loader, model, config.DEVICE)
